...
if __name__ == '__main__':
  ...
  ap = AudioProcessor(
      ...
      output_representation=output_representation)
  model = load_model('checkpoints_162/ep-063-vl-0.2273.hdf5',
  ...
  pd.DataFrame({'fname': fns, 'label': wanted_labels}).to_csv(
      'submission_162_tta_flsl.csv',
      index=False, compression=None)

  pd.DataFrame({'fname': fns, 'label': labels}).to_csv(
      'submission_162_tta_flsl_all_labels.csv',
      index=False, compression=None)
  ...
  all_data.to_csv(
      'submission_162_tta_flsl_all_labels_probs.csv',
      index=False, compression=None)
  print("Done!")
