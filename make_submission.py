...
if __name__ == '__main__':
  ...
  model = load_model('checkpoints_097/ep-078-vl-0.2130.hdf5',
  ...
  pd.DataFrame({'fname': fns, 'label': wanted_labels}).to_csv(
      'submission_097_leftloud_tta.csv', index=False, compression=None)

  pd.DataFrame({'fname': fns, 'label': labels}).to_csv(
      'submission_097_leftloud_tta_all_labels.csv',
  ...
  all_data.to_csv(
      'submission_097_leftloud_tta_all_labels_probs.csv',
      index=False, compression=None)
  print("Done!")
